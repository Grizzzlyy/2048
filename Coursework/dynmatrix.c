...
void copyMatrix(unsigned short** sMatrix, unsigned short** dMatrix, unsigned short size)
{
    for (unsigned short i = 0; i < size; i += 4)
    {
        for (unsigned short j = 0; j < size; j += 4)
        {
            ...
            dMatrix[i + 1][j + 1] = sMatrix[i + 1][j + 1];
            dMatrix[i + 2][j + 2] = sMatrix[i + 2][j + 2];
            dMatrix[i + 3][j + 3] = sMatrix[i + 3][j + 3];
        }
    }
}
...
bool cmpMatrix(unsigned short** matrix1, unsigned short** matrix2, int size)
{
    for (int i = 0; i < size; i += 2)
    {
        ...
        {
            if (matrix1[i][j] != matrix2[i][j] || matrix1[i+1][j+1] != matrix2[i+1][j+1] )
                return 1;
        }
    }
    return 0;
...
void printMatrix(unsigned short** matrix, unsigned short size)
{
    for (int i = 0; i < size; i += 4)
    {
        ...
        {
            printf("%hu %hu %hu %hu ", matrix[i][j], matrix[i + 1][j + 1], matrix[i + 2][j + 2], matrix[i + 3][j + 3]);
        }
        printf("\n");
    }
}